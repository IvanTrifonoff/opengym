...
## Инструкция тренерского портала (встроенная)

В шапке `/trainer` появилась кнопка **«Инструкция»** — открывает нижний шит
с руководством по порталу тренера: вкладка «Спортсмены» (статусы, фильтры,
карточка), вкладка «Добавить» (ссылка-приглашение с авто-привязкой и поиск
существующего), программы спортсмена (редактор плана + прогресс по весам и
подходам), календарь (часы работы с несколькими интервалами и выходными,
сетка недели, заявки, «Записать»), а также практические советы и заметка о
разграничении доступа.

- `frontend/src/components/TrainerHelp.jsx` — компонент шита
  (`trainerHelpSheet()` через `useUI.openSheet`), рендерится штатным
  `<Modals />` админ-роута.
- Кнопка добавлена в шапку главного экрана портала рядом с «Справка» и
  «Выйти».
## Инструкция о баллах и наградах для спортсменов (в приложении)

В приложении спортсмена (Настройки → Лояльность) в карточке баланса
появилась кнопка «?» — открывает нижний шит «Баллы и награды»: как
начисляются баллы (посещения, тренировки, серии, рефералы), что такое
награды, как обменять баллы (3 шага) и что означают статусы заявок
(ожидает сотрудника / выполнено / отклонено с возвратом баллов).

- `frontend/src/components/LoyaltyInfo.jsx` — компонент шита
  (`loyaltyInfoSheet()` через `useUI.openSheet`), рендерится штатным
  `<Modals />` приложения. Все строки — через `t()` (база — английский,
  переводы в `locales/ru.js`).
- **Бонус**: вся секция лояльности в приложении была без перевода — добавлен
...
## Уведомления о начислении баллов лояльности (авто, через outbox)

Раньше push спортсмену уходил только если в правиле явно задано действие
«Уведомление». Теперь **при начислении баллов** спортсмен автоматически
получает push-уведомление через существующий `loyalty_outbox`:
«+15 баллов — Ежедневное посещение» (сообщение локализуется по языку
приложения спортсмена, с правильным склонением «балл/балла/баллов»).
Если в правиле задано своё уведомление — используется оно, авто-дубликат
не создаётся.

- `api/admin-db.js`:
  - `pointsMessage(amount, ruleName, lang)` — локализованное сообщение;
  - `applyLoyaltyRules({…, lang='en'})` — в ветке `points`, когда баллы
    реально начислены (не дубликат события), вставляет строку в
    `loyalty_outbox`, если у правила нет своего notification-действия;
  - `acceptLoyaltyEvent({…, lang})` — пробрасывает язык.
- `api/server.js` — `langOf(userId)` читает `lang` из state-файла
  спортсмена; передаётся в оба вебхука (loyalty и access).
- Фронтенд: подсказка в редакторе правила (AdminApp.jsx), обновлены
  инструкции LoyaltyHelp.jsx и LoyaltyInfo.jsx (+ переводы в ru.js).

Доставка — как и раньше: `dispatchOutbox` сразу в вебхуке и каждые 30 с.

Проверено end-to-end: правило «+15 за посещение» без уведомления → событие
через вебхук → `points:15, notified:1`; outbox `delivered:true` на реальном
FCM-эндпоинте; сообщения: ru «+15 баллов — …», en «+15 points — …»;
склонения 1/2/5/11/21/25/100 — корректны.
## Мониторинг доставки push-уведомлений

`server.js` считает статистику доставки пушей (в памяти) и умеет алертить о
сбоях двумя способами.

**Счётчики**: `sent` / `failed` / `expired` + буфер последних 50 сбоев
(хост, статус, текст ошибки, время). Мёртвые подписки (404/410) — обычная
очистка, считаются в `expired` и не считаются сбоем.

**1. Баннер в админке** (всегда): на вкладке «Обзор» — плитка «Пуши»
(ok/сбои) и красная карточка «Сбои доставки push-уведомлений» с числом не
отправленных за 24 ч, последним сбоем и кнопкой «Сбросить» (owner/manager).
Данные — `GET /api/admin/push/status`, сброс — `POST /api/admin/push/status/reset`.

**2. Вебхук-алерт** (опционально): `PUSH_ALERT_WEBHOOK` в `.env` — POST JSON
при сбое:
```json
{ "type": "push_delivery_alert", "origin": "https://gym.trfnv.ru",
  "failed": 1, "last": { "at": "…", "host": "fcm.googleapis.com", "status": null, "error": "…" } }
```
Дебаунс: один алерт в 5 минут; когда доставка снова работает 5 минут —
`push_delivery_recovered`. Пример в `.env.example`.
...
## Бейдж на иконке: сброс при просмотре pending-элементов

Бейдж (Badging API) больше не «залипает»: он показывает только то, что
спортсмен ещё **не видел**.

**Как работает:**
- У pending-наград (`loyalty_redemptions`) и pending-записей
  (`coach_bookings`) появилась колонка `viewed_at` (миграция — автоматически
  при старте api).
- Когда спортсмен открывает раздел, где эти pending-элементы показаны
  (**Настройки → Лояльность** — история наград, **шит «Мой тренер»** — мои
  записи), клиент вызывает `POST /api/badge/seen` → все его pending-элементы
  помечаются просмотренными, бейдж пересчитывается и исчезает.
- `refreshBadge()` (и `sw.js` в push-событии) считают только pending-элементы
  без `viewed_at`: `unread-уведомления + непросмотренные pending-награды +
  непросмотренные pending-записи`.
- Новый pending-элемент (новая награда/новая запись) снова показывает бейдж,
  пока спортсмен не откроет соответствующий раздел.

**API:** `POST /api/badge/seen` — пометить все pending награды и записи
пользователя просмотренными (идемпотентно).

Проверено: 1 непросмотренная pending-награда → `setAppBadge(1)`; после
открытия раздела → `clearAppBadge()`; новый pending снова даёт бейдж.
Тестовые данные убраны.
## Бейдж-счётчик новых заявок в тренерском портале

В `/trainer` теперь видно, сколько заявок на тренировки ждут подтверждения —
не заходя во вкладку «Календарь».

**Как работает:**
- В шапке портала — колокольчик с числом pending-заявок (до «9+»); клик
  переводит на вкладку «Календарь».
- На вкладке «Календарь» в таб-баре — такой же счётчик.
- Счёт обновляется: при загрузке портала, каждые 30 секунд (поллинг), и
  сразу после действий во вкладке «Календарь» (подтвердить/отклонить/
  создать запись — событие `trainer-bookings-changed`).
- Данные — существующий `GET /api/admin/trainer/bookings` (тренер видит
  только свои), считается `status === 'pending'`.

Проверено end-to-end от имени тренера: с pending-заявкой колокольчик и
таб показывают «1»; после «Подтвердить» бейдж исчезает; 0 JS-ошибок.
Тестовые данные убраны.
## Бейдж на иконке у тренера (Badging API)

Иконка установленного приложения синхронизируется с тренерским порталом:
пока у тренера есть pending-заявки на тренировки, на иконке горит их число.

**Как работает:**
- `lib/badge.js`: общий `setBadge(n)` (set/clear), `refreshTrainerBadge()`
  считает pending-заявки через `GET /api/admin/trainer/bookings` (тренер
  видит только свои, `adminsid`-сессия) и ставит бейдж на иконку.
- В `/trainer` `refreshTrainerBadge()` вызывается вместе с обновлением
  счётчика в портале: при загрузке, поллинг каждые 30 с и сразу после
  подтверждения/отклонения/создания записи (событие
  `trainer-bookings-changed`).
- При выходе из портала бейдж очищается (`clearBadge`) — приложение
  спортсмена пересчитает свой бейдж при следующем фокусе.

Проверено: 1 pending-заявка → `setAppBadge(1)` (и колокольчик «1»);
после «Подтвердить» → `clearAppBadge()`, бейджа нет; 0 JS-ошибок.
Тестовые данные убраны.
## Push-уведомления тренеру о новых заявках

Когда спортсмен записывается на тренировку, тренер получает push.

**Как работает:**
- В шапке портала `/trainer` появилась кнопка **«Вкл. пуши»** (после
  включения — «Пуши: вкл»). Тренер подписывается на Web Push прямо из
  портала: `POST /api/push/subscribe` теперь принимает и админ-сессию
  (`adminsid`), подписка хранится как `admin:<id>`.
- При создании заявки спортсменом (`POST /api/trainer/book`,
  статус `pending`) сервер шлёт тренеру push:
  «Новая заявка на тренировку · Имя · дата время · заметка» (`tag:
  booking-<id>`).
- Если тренер не подписан — тихий no-op (доставка статистикой
  `/api/admin/push/status`, мониторинг и алерты работают как раньше).
- `POST /api/push/test` и `/api/push/unsubscribe` также понимают
  админ-сессии (кнопка использует их для проверки/отключения).

Проверено end-to-end: тренер включил пуши в портале (реальная FCM-подписка
`admin:hj0F…`), спортсмен создал заявку через `/api/trainer/book` →
`sent: 1, failed: 0` — FCM принял push; тестовые данные убраны.
## Push-уведомления спортсмену о статусе записи к тренеру

Когда тренер отвечает на заявку спортсмена, тот получает push — и запись
появляется в центре уведомлений приложения.

**Как работает:**
- `POST /api/admin/trainer/bookings/status` при смене статуса на
  `confirmed` / `rejected` / `cancelled` / `done` шлёт спортсмену push:
  - заголовок «Запись к тренеру» / «Trainer booking»;
  - тело: имя тренера · «Тренер подтвердил запись / отклонил заявку /
    запись отменена / тренировка отмечена выполненной: ДД.ММ · ЧЧ:ММ»;
  - язык — по настройке языка приложения спортсмена (`langOf`, ru/en);
  - `tag: booking-<id>` — повторные статусы заменяют уведомление, не копятся.
- Сообщение дополнительно сохраняется в `app_notifications` (ид `bk-<id>`,
  идемпотентно) — видно на странице «Уведомления» в приложении и
  учитывается бейджем.
- `saveNotification()` добавлена в `admin-db.js` (использует общий `pool`).

Проверено end-to-end: спортсмен подписан на FCM → заявка →
«Подтвердить» → `sent: 1, failed: 0`, в центре уведомлений:
«Andrey · Тренер подтвердил запись: 28.08 · 16:00» (ru). Также проверен
reject. Тестовые данные убраны.
## Центр уведомлений тренера

У тренера появился свой центр уведомлений — как у спортсменов.

**Как работает:**
- Каждая новая заявка спортсмена сохраняется в `app_notifications` под
  `user_id = 'admin:<тренер>'` (ид `tbk-<booking>`): заголовок «Новая
  заявка на тренировку», тело «Спортсмен · ДД.ММ · ЧЧ:ММ — заметка».
- Страница `/trainer/notifications` — список уведомлений, при открытии всё
  помечается прочитанным (`POST /api/admin/notifications/read`).
- В шапке портала колокольчик показывает число непрочитанных (клик — в
  центр); бейдж на иконке (Badging API) синхронизирован с этим же числом
  (`refreshTrainerBadge` читает `/api/admin/notifications`).
- Счётчик на вкладке «Календарь» по-прежнему показывает число именно
  pending-заявок (быстрый взгляд на очередь).
- Уведомления о статусах (подтверждено/отклонено), которые тренер сам
  выставил, в центр тренера не пишутся (только спортсмену) — центр
  отражает входящие заявки.

**API (админ-сессия `adminsid`):**
- `GET /api/admin/notifications` — список для текущего тренера;
- `POST /api/admin/notifications/read` `{id?}` — пометить одно/все.

Проверено end-to-end: заявка спортсмена → строка `tbk-…` под
`admin:hj0F…` → колокольчик «1» → страница центра со списком → авто-read
(`unread: 0`); 0 JS-ошибок. Тестовые данные убраны.
## Оптимизация старта: инструкции упражнений грузятся лениво

Запуск приложения ускорен: больше не скачивается и не парсится ~1 МБ
инструкций упражнений при каждом старте.

**Как было:** `setLang()` при старте подгружал три ru-пака — переводы UI
(22 КБ), имена упражнений (16 КБ) и **все инструкции упражнений (~205 КБ
gzip / 1 МБ текста)** — хотя инструкции нужны только при открытии карточки
упражнения.

**Как стало:** `i18n.js` — `setLang()` грузит только переводы и имена;
инструкции — через `ensureInstr()` **по требованию**, когда спортсмен
открывает карточку упражнения (`ExerciseDetail` → `useEffect`). Пока пак
ещё не загрузился, шаги показываются из английских данных (`ex.st`), затем
перерисовываются в переводе. Языки без пакета (en, de, pt) — как раньше,
английские шаги. При смене языка пакет перезагружается под новый язык.

Проверено (холодный старт, сеть): при старте грузятся только
`index.js + css + ru-переводы + ru-имена` (инструкции **не** запрашиваются);
после клика по упражнению подгружается `ru-*.js` инструкций, и шаги
отображаются («Техника выполнения» → «Встаньте, поставив ноги на ширине
плеч…»). 0 JS-ошибок.
## Оптимизация старта: интерфейс раньше истории (early render)

Приложение показывает интерфейс сразу, а полную историю тренировок
подгружает в фоне.

**Как было:** `boot()` ждал две сетевые операции подряд: `/api/me`
(кто я) → `/api/data` (вся история тренировок, заметный JSON) — и только
потом ставил `ready: true`, т.е. весь этот путь блокировал первый экран.

**Как стало:** `ready: true` ставится сразу после `/api/me` — приложение
рендерится мгновенно (с локальной копией данных), а `pullState()` идёт в
фоне и обновляет стор по мере загрузки (`persist` → re-render). Для
вернувшихся пользователей интерфейс и так появлялся из localStorage —
теперь не ждём и `pullState`.

Проверено: при искусственной задержке `/api/data` на 2.5 с интерфейс
(шапка «Привет, …», неделя, карточки) появляется за ~430 мс, после
задержки история доезжает и стор обновляется; гость без сессии по-прежнему
видит экран входа (не пустой интерфейс); 0 JS-ошибок.
## Кастомные упражнения изолированы по пользователю (клиентский кеш)

**Проблема.** Кеш данных приложения в localStorage хранился под одним ключом `gym_state_v1` на всех пользователей устройства. При смене аккаунта (два спортсмена на одном устройстве, истёкшая сессия + новый вход) новый пользователь наследовал чужой кеш, и `pullState` мог **запушить чужие данные (включая кастомные упражнения) в его аккаунт на сервере**.

**Фикс (frontend).**
- Кеш теперь привязан к пользователю: `gym_state_<userId>` (гости — `gym_state_guest`), флаг несинхронизированных изменений тоже: `gym_dirty_<userId>`.
- `setUser()` перезагружает state из кеша **нового** пользователя при смене аккаунта — чужие данные не переносятся ни в UI, ни на сервер.
- Исключение: регистрация нового профиля из гостевого режима (`keepLocal: true`) — гостевые данные переносятся в новый аккаунт, как раньше.
- Выход из аккаунта удаляет только кеш вышедшего пользователя.

**Проверено end-to-end** (реальный Chrome, один профиль): Testuser1 создал кастомное упражнение через UI → Trfnv открыл приложение в том же профиле → в UI кастома нет, `customEx` в его server-state пуст, ключи localStorage раздельные (`gym_state_VAd_…` / `gym_state_Cl4u…`), 0 JS-ошибок. Тестовые данные убраны.

**Замечание по эксплуатации.** Ранние тесты до фикса могли затреть state одного пользователя данными другого. При подозрении: данные восстанавливаются автоматически с устройства владельца (клиент пушит свой кеш, если он новее серверного) либо из бэкапа `backup.sh`.

---

## 15. Исправление push-уведомлений: навигация из колокольчика + health-эндпоинт

### Что исправлено

**sw.js** — нажатие на push-уведомление теперь открывает страницу уведомлений:
-  передаёт  →  (SPA-навигация, без перезагрузки) или  (если окно закрыто)
- Push-хендлер передаёт  в уведомление

**server.js** —  автоматически подставляет  во все push-пейлоады:
- Новый эндпоинт:  — возвращает количество подписок, статистику доставки (требует сессию)
- Полезно для отладки: {"error":"not signed in"}

### Цикл push-уведомлений (Android Chrome)

1. Пользователь → Настройки → Уведомления → вкл →  подписывается через FCM
2. Сервер хранит подписку в  (в )
3. При отправке push → SW получает  → показывает уведомление с 
4. Пользователь нажимает →  →  → открывается 
5. Страница помечает уведомления прочитанными → бейдж очищается

### Известное поведение на Android Chrome

- Push требуют установки приложения (Add to Home Screen) для стабильной доставки в фоне
- Chrome может throttлить уведомления от неустановленных PWA в режиме экономии батареи
- Если подписки потеряны (сброс данных), пользователь должен повторно включить в Настройках
- Используйте  для проверки статуса подписок
...
**sw.js** — нажатие на push-уведомление теперь открывает страницу уведомлений:
- notificationclick передаёт URL через data-атрибут уведомления → SPA-навигация без перезагрузки
- Push-хендлер передаёт URL в data уведомления (по умолчанию /notifications)

**server.js** — sendPush автоматически подставляет url: /notifications во все push-пейлоады:
- Новый эндпоинт: GET /api/push/health — возвращает количество подписок и статистику доставки
- Полезно для отладки: curl gym.trfnv.ru/api/push/health с кукой сессии

### Известное поведение на Android Chrome

- Push требуют установки приложения (Add to Home Screen) для стабильной доставки в фоне
- Chrome может throttлить уведомления от неустановленных PWA в режиме экономии батареи
- Если подписки потеряны, пользователь должен повторно включить в Настройках
- Используйте /api/push/health для проверки статуса подписок
