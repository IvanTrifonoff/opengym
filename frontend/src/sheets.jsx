...
function ExercisePicker({ onPick, close }) {
  ...
  let base = all.filter(e =>
    ...
    // Поиск идёт и по английскому, и по переведённому названию (exName) —
    // в русском интерфейсе «жим», «тяга», «присед» находят упражнение сразу.
    (!ql || e.n.toLowerCase().includes(ql) || exName(e).toLowerCase().includes(ql) || e.tg.includes(ql) || e.eq.includes(ql) || (e.desc || '').toLowerCase().includes(ql)))
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'impulsegym-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own impulseGym — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
