...
function Loyalty({ canEdit }) {
  ...
  return <>
    <div className="row between" style={{ marginBottom: 10 }}><div><h2 style={{ margin: 0 }}>Программа лояльности</h2><div className="sub">Правила применяются к событиям без изменения кода</div></div>{canEdit && <Button variant="primary" size="sm" icon="plus" onClick={() => setEditing({})}>Правило</Button>}</div>
...
function Rewards({ canEdit }) {
  ...
  return <>
    <div className="row between" style={{ marginBottom: 10 }}><div><h2 style={{ margin: 0 }}>Награды</h2><div className="sub">Каталог, стоимость и способ выдачи</div></div>{canEdit && <Button variant="primary" size="sm" icon="plus" onClick={() => setForm(blankReward())}>Награда</Button>}</div>
...
function Staff({ admin }) {
  ...
  return <>
    ...
    <ErrorLine error={error} />
    <div className="list">{staff.map(s => <div className="item" key={s.id}><div className="grow"><div className="tt">{s.name} {s.disabled && <span className="tag" style={{ color: 'var(--red)' }}>off</span>}</div><div className="ss">{s.role} · {s.passkeys} passkey</div></div><span className="tag acc">{s.role}</span></div>)}</div>
  </>
}
