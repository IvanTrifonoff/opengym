...
function Staff({ admin }) {
  const [staff, setStaff] = useState([]); const [name, setName] = useState(''); const [role, setRole] = useState('trainer'); const [branchKey, setBranchKey] = useState(''); const [branches, setBranches] = useState([]); const [invite, setInvite] = useState(null); const [error, setError] = useState('')
  ...
  useEffect(() => { load(); }, [])
  // Филиалы для селектора в форме инвайта. Бэкенд сам скоупит список по club_id
  // (scopeBranches) — чужих филиалов в ответе нет, селектор физически не может
  // отправить чужой branch_key.
  useEffect(() => { api('/api/admin/branches').then(d => setBranches(d.branches || [])).catch(() => {}) }, [])
  const makeInvite = () => api('/api/admin/staff/invite', { method: 'POST', body: JSON.stringify({ name, role, branch_key: branchKey || null }) }).then(d => { setInvite(d.invite); setName(''); load() }).catch(e => setError(e.message))
  ...
  return <>
    ...
    {canManage && <div className="card"><h3 style={{ marginTop: 0 }}>Пригласить сотрудника</h3><div className="row" style={{ gap: 8 }}><input className="field" value={name} onChange={e => setName(e.target.value)} placeholder="Имя сотрудника" /><select className="field" value={role} onChange={e => setRole(e.target.value)}>{roles.filter(r => r !== 'owner').map(r => <option key={r}>{r}</option>)}</select><select className="field" value={branchKey} onChange={e => setBranchKey(e.target.value)} title="Филиал"><option value="">Филиал: все</option>{branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select><Button variant="primary" size="sm" onClick={makeInvite} disabled={!name.trim()}>Создать</Button></div><div className="small dim" style={{ marginTop: 6 }}>Филиал «все» = сотрудник клуба (видит аналитику всех залов клуба); конкретный филиал — только свой зал.</div>{invite && <div className="small" style={{ marginTop: 10 }}>Код: <b>{invite.code}</b><br />Ссылка: <span className="dim">{link}</span><button className="btn xs plain" onClick={() => navigator.clipboard?.writeText(link)}>Копировать</button></div>}</div>}
    <ErrorLine error={error} />
...
function AdminDashboard({ admin, onLogout }) {
  ...
  const [sp] = useSearchParams(); const [staff, setStaff] = useState([]); const [rules, setRules] = useState([]); const [push, setPush] = useState(null); const [hasSeed, setHasSeed] = useState(false)
  ...
  useEffect(() => {
    api('/api/admin/staff').then(d => { setStaff(d.admins || []); setHasSeed(!!d.hasSeed) }).catch(() => {})
