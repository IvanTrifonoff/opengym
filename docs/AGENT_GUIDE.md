...
| **trainer** | свои атлеты: программы, расписание/брони, заявки на запись, удержание своих атлетов | `/trainer` (единственная поверхность; `/admin*` для тренера закрыты) |
...
> | `retention` | тренер | `/trainer` → «Удержание» (свои атлеты) + фокус на спортсмене |
> | `retention-net` | владелец/менеджер | `/admin/analytics` → «Удержание» |
> | `promo_lead` | владелец/менеджер | `/admin?tab=leads` + фокус на заявке |
>
> **ВАЖНО — маршрут зависит от РОЛИ получателя, а не только от kind.** Тренер
> всегда остаётся в своём портале `/trainer` (отвечает только за своих атлетов);
> владелец/менеджер работают в `/admin*` (атлеты клуба). Никогда не отправляй
> тренера на `/admin/analytics` или в панель владельца — это его «запирает»:
> оттуда нет обратного пути в портал. Обратное тоже верно: владельца не води
> по `/trainer`.
